...
const navigationMenuTriggerStyle = cva(
	[
		'group inline-flex h-9 min-w-[88px] items-center justify-center',
		'rounded-2xl', // Modern, soft rectangle—less "pill"
		'px-6 py-2.5', // Spacious, premium
		'text-sm font-extrabold text-[var(--color-dark-slate)]',
		' transition-all duration-300 ease-in-out',
		'outline-none border-none', // Absolutely NO border
		'hover:bg-[var(--color-primary-light)] hover:text-[var(--color-background)]',
		' hover:-translate-y-0.5',
		'focus:bg-[var(--color-primary)] focus:text-[var(--color-background)]',
		'focus-visible:ring-2 focus-visible:ring-[var(--color-primary-soft)] focus-visible:ring-offset-2',
		'disabled:pointer-events-none disabled:opacity-50',
		// Open state (dropdown active)
		'data-[state=open]:bg-[var(--color-primary)]',
		'data-[state=open]:text-[var(--color-background)]',
		'data-[state=open]:hover:bg-[var(--color-primary-dark)]',
	].join(' ')
);
...
function NavigationMenuContent({
	...
	return (
		<NavigationMenuPrimitive.Content
			...
			className={cn(
				// keep absolute; avoid forcing width here
				'data-[motion^=from-]:animate-in data-[motion^=to-]:animate-out',
				'data-[motion^=from-]:fade-in data-[motion^=to-]:fade-out',
				'data-[motion=from-end]:slide-in-from-right-52 data-[motion=from-start]:slide-in-from-left-52',
				'data-[motion=to-end]:slide-out-to-right-52 data-[motion=to-start]:slide-out-to-left-52',
				'top-0 left-0 p-2 pr-2.5 md:absolute',
				// let viewport=false styling only apply when actually disabling viewport
...
function NavigationMenuViewport({
	...
	return (
		<div
			className={cn(
				// full-width container that centers the viewport
		...
		>
			<NavigationMenuPrimitive.Viewport
				...
				className={cn(
					...
					'w-[110vw] max-w-none overflow-hidden rounded-md border bg-popover text-popover-foreground shadow',
